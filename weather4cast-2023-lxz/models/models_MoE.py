...
class WeatherFusionNet_stage1(BaseLitModule):
    def __init__(self, UNet_params, config):
        ...
        self.phydnet = PhyDNetWrapper("models/configurations/phydnet.yaml", ckpt_path="")
    ...
    def forward(self, x, return_inter=False):
        ...
        ### seems like sat2rad() should return a Tensor but it is returning tuple so it is making issues
        ### what they are doing here is refactoring x to have 4 dimensions (which is same as phydnet_out) instead of 5 dims
        #     ss = self.sat2rad(x.swapaxes(1, 2))
        #     sat = torch.stack(ss[1:], dim=0)
        #     sat2rad_out = sat.reshape(x.shape[0], 4, x.shape[-2], x.shape[-1])
        # sat2rad_out = torch.stack(self.sat2rad(x.swapaxes(1, 2))[1:], dim=0) ### for testing only

        self.phydnet.eval()
        with torch.no_grad():
            print(type(self.phydnet(x.swapaxes(1, 2))))
